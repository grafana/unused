...
func (d *Disk) ID() string { return fmt.Sprintf("gcp-disk-%d", d.Id) } // TODO remove prefix
...
func (d *Disk) CreatedAt() time.Time {
	...
	c, _ := time.Parse(time.RFC3339, d.CreationTimestamp)
...
func (d *Disk) LastUsedAt() time.Time {
	...
	// it's safe to assume GCP will send a valid timestamp
	t, _ := time.Parse(time.RFC3339, d.LastDetachTimestamp)
	return t
}
